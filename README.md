...
2) *cd* to the `packer-ansible-ec2` directory and then `git checkout` the build branch of interest
3) Either edit `packer-build.json` directly or copy to new json file and edit new file  
<tab>modify the following variables:
* `ami_name`: "Name of AMI 1.0 {{isotime `2006-01-02-150405`}}" (default AMI name will include time stamp of build launch)
